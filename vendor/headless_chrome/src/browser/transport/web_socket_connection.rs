...
// uxlint patch: was 100ms — the reader holds the connection MUTEX during its blocking
// read, so the writer waits for WouldBlock+park before it can send. Every method call
// synced to that ~100ms cadence (measured: 103ms for evaluate("1")). 5ms bounds the
// mutex hold; the writer gets in within a few ms and calls run at wire speed.
                ...
                Err(err) => match err {
                    tungstenite::Error::Io(err) => {
                        if matches!(
                            ...
                            // uxlint patch: park releases the mutex — keep it short so a
                            // response arriving mid-park is read promptly, not 100ms later.
