...
impl Transport {
        ...
        C: Method + serde::Serialize,
    {
        ...
        // response finds a dropped receiver, which the incoming-message loop used to
        // treat as fatal, killing the whole browser connection over one slow method.
        ...
        parse_response::<C::ReturnObject>(response?)
    }
                ...
                match messages_rx.recv_timeout(idle_browser_timeout) {
                    ...
                    Ok(message) => match message {
                        ...
                        Message::Response(response_to_browser_method_call) => {
                            // uxlint patch: a response nobody is waiting for (the caller
                            // timed out) is noise, not a reason to kill the connection.
                            if waiting_call_registry
                                .resolve_call(response_to_browser_method_call)
                                .is_err()
                            {
                                warn!("Dropped a browser response whose caller gave up waiting");
                            }
                        }
                        ...
                        Message::Event(browser_event) => match browser_event {
                                ...
                                match msg_res {
                                    Ok(target_message) => match target_message {
                                        Message::Event(target_event) => {
                                            // uxlint patch: an event for a dropped tab
                                            // (e.g. one replaced after a wedge) must not
                                            // panic the loop thread — just discard it.
                                                ...
                                                .get(&ListenerId::SessionId(session_id))
                                            {
                                                if tx.send(target_event).is_err() {
                                                    warn!("Dropped an event for a listener that went away");
                                                }
                                            }
                                        }

                                        Message::Response(resp) => {
                                            // uxlint patch: same as the browser-level case —
                                            // a late response to a timed-out caller is noise.
                                            if waiting_call_registry.resolve_call(resp).is_err() {
                                                warn!("Dropped a tab response whose caller gave up waiting");
                                            }
                                        }
                                        Message::ConnectionShutdown => {}
                                    },
                            ...
                            _ => {
                                // uxlint patch: the browser-event listener going away must
                                // not tear down the transport every tab depends on — drop
                                // the listener entry and keep serving method calls.
                                let mut listeners_guard = listeners.lock().unwrap();
                                if let Some(tx) = listeners_guard.get(&ListenerId::Browser) {
                                    if tx.send(browser_event.clone()).is_err() {
                                        warn!("Browser-event listener went away — unsubscribing it");
                                        listeners_guard.remove(&ListenerId::Browser);
                                    }
                                }
                            }
                        },
